...
hardware). The gallery page shows its source plus real, pre-rendered
benchmark plots generated by manually running it on real hardware - see
...
# Example output
# ---------------
# Generated by manually running this script on real hardware with
# ``--models sup_hopf mpr --no-jax`` (not re-executed on every docs build -
# see the module docstring). CUDA numbers are real (numba.cuda); JAX was
# skipped since it's an optional dependency not installed on RTD.
#
# .. figure:: /examples/simulator_models/reference_images/benchmark_all_models_001.png
#    :alt: SupHopf sweep benchmark across backends
#
#    SupHopf
#
# .. figure:: /examples/simulator_models/reference_images/benchmark_all_models_002.png
#    :alt: MPR sweep benchmark across backends
#
#    MPR
...
# This script's own directory must be on sys.path too, for the sibling
# benchmark_all_backends import below (sphinx-gallery execs files with cwd
# already set to this directory, but plain `python` invocation needs this).
