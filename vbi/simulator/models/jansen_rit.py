...
#
# Long-range coupling: use CouplingSpec(kind="difference") so that
#   c_y1 = W @ (y1 - y2)  (raw weighted PSP difference, no gain scaling).
# The model dfun then applies G * S(c_y1), matching the legacy numba convention:
#   G * S(sum_j W_ij * (y1_j - y2_j)).
jansen_rit = ModelSpec(
    ...
    dfun_str={
        ...
        "y4": "A * a * (mu + a_2*J*(2*nu_max/(1+exp(r*(v0 - a_1*J*y0)))) + G*(2*nu_max/(1+exp(r*(v0 - c_y1))))) - 2*a*y4 - a**2*y1",
    ...
    dfun_latex={
        ...
        "y4": r"Aa\!\left(\mu + \alpha_2 J\,S(\alpha_1 J y_0) + G\,S(c_{y_1})\right) - 2a\,y_4 - a^2 y_1",
    ...
    latex_notes=(
        ...
        r"Long-range: $c_{y_1} = \sum_j W_{ij}(y_{1,j}-y_{2,j})$ via \texttt{CouplingSpec(kind='difference')}; "
        r"gain $G$ and sigmoid applied in $\dot{y}_4$."
    ),
)
