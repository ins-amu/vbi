"""
Tests for InferencePipeline.from_config - Step 5 of MI6.
"""
import json
import numpy as np
import pytest
...
class TestFromConfigDict:
    ...
    def test_from_dict_creates_inference_object(self, tmp_path):
        ...
        inf = InferencePipeline.from_config(_minimal_config(conn))
        assert isinstance(inf, InferencePipeline)
    ...
    def test_de_type_set(self, tmp_path):
        from vbi.inference._inference_pipeline import _snpe_de_key

        conn = _write_connectivity(tmp_path)
        inf = InferencePipeline.from_config(_minimal_config(conn))
        assert _snpe_de_key(inf._snpe) == "maf"
...
class TestFromConfigFile:
    ...
    def test_from_json_file(self, tmp_path):
        ...
        inf = InferencePipeline.from_config(str(json_path))
        assert isinstance(inf, InferencePipeline)
    ...
    def test_from_yaml_file(self, tmp_path):
        ...
        inf = InferencePipeline.from_config(str(yaml_path))
        assert isinstance(inf, InferencePipeline)
    ...
    def test_unknown_model_raises(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="Unknown model"):
            InferencePipeline.from_config(cfg)
